...
(function make_xlsx(XLSX) {
    ...
    function char_codes(data) { var o = []; for (var i = 0, len = data.length; i < len; ++i)
        o[i] = data.charCodeAt(i); return o; }
    ...
    var make_ssf = function make_ssf(SSF) {
        SSF.version = '0.9.3';
        function _strrev(x) { var o = "", i = x.length - 1; while (i >= 0)
            o += x.charAt(i--); return o; }
        function fill(c, l) { var o = ""; while (o.length < l)
            o += c; return o; }
        ...
        function pad0r(v, d) { if (v > p2_32 || v < -p2_32)
            return pad0r1(v, d); var i = Math.round(v); return pad0r2(i, d); }
            ...
            switch (type) {
                ...
                case 115:/* 's' seconds */ 
                    if (val.u === 0)
                        switch (fmt) {
                            case 's':
                            case 'ss': return pad0(val.S, fmt.length);
                            case '.0':
                            case '.00':
                            case '.000':
                        }
        ...
        var write_num = (function make_write_num() {
            ...
            function flr(val) { if (val < 2147483647 && val > -2147483648)
                return "" + (val >= 0 ? (val | 0) : (val - 1 | 0)); return "" + Math.floor(val); }
        ...
        function eval_fmt(fmt, v, opts, flen) {
            ...
            for (i = 0; i < out.length; ++i) {
                switch (out[i].t) {
                    ...
                    case '?':
                        jj = i + 1;
                        while (out[jj] != null && ((c = out[jj].t) === "?" || c === "D" ||
        ...
        SSF.load_table = function load_table(tbl) { for (var i = 0; i != 0x0188; ++i)
            if (tbl[i] !== undefined)
                SSF.load(tbl[i], i); };
    ...
    function parsexmltag(tag, skip_root) {
        ...
        if (m)
            for (i = 0; i != m.length; ++i) {
                ...
                if (j === q.length) {
                    if (q.indexOf("_") > 0)
                        q = q.substr(0, q.indexOf("_")); // from ods
                    z[q] = v;
                }
                else {
    ...
    __toBuffer = ___toBuffer = function toBuffer_(bufs) { var x = []; for (var i = 0; i < bufs[0].length; ++i) {
        x.push.apply(x, bufs[0][i]);
    } return x; };
    var __utf16le, ___utf16le;
    __utf16le = ___utf16le = function utf16le_(b, s, e) { var ss = []; for (var i = s; i < e; i += 2)
        ss.push(String.fromCharCode(__readUInt16LE(b, i))); return ss.join(""); };
    var __hexlify, ___hexlify;
    __hexlify = ___hexlify = function hexlify_(b, s, l) { return b.slice(s, (s + l)).map(function (x) { return (x < 16 ? "0" : "") + x.toString(16); }).join(""); };
    var __utf8, ___utf8;
    __utf8 = ___utf8 = function (b, s, e) { var ss = []; for (var i = s; i < e; i++)
    ...
    if (has_buf) {
        __utf16le = function utf16le_b(b, s, e) { if (!Buffer.isBuffer(b))
            return ___utf16le(b, s, e); return b.toString('utf16le', s, e); };
        __hexlify = function (b, s, l) { return Buffer.isBuffer(b) ? b.toString('hex', s, s + l) : ___hexlify(b, s, l); };
        __lpstr = function lpstr_b(b, i) { if (!Buffer.isBuffer(b))
            return ___lpstr(b, i); var len = b.readUInt32LE(i); return len > 0 ? b.toString('utf8', i + 4, i + 4 + len - 1) : ""; };
        __lpwstr = function lpwstr_b(b, i) { if (!Buffer.isBuffer(b))
            return ___lpwstr(b, i); var len = 2 * b.readUInt32LE(i); return b.toString('utf16le', i + 4, i + 4 + len - 1); };
        __lpp4 = function lpp4_b(b, i) { if (!Buffer.isBuffer(b))
            return ___lpp4(b, i); var len = b.readUInt32LE(i); return b.toString('utf16le', i + 4, i + 4 + len); };
        __8lpp4 = function lpp4_8b(b, i) { if (!Buffer.isBuffer(b))
        ...
        __double = function double_(b, i) { if (Buffer.isBuffer(b))
            return b.readDoubleLE(i); return ___double(b, i); };
    ...
    function parsenooplog(blob, length) { if (typeof console != 'undefined')
        console.log(blob.slice(blob.l, blob.l + length)); blob.l += length; }
    ...
    function decode_col(colstr) { var c = unfix_col(colstr), d = 0, i = 0; for (; i !== c.length; ++i)
        d = 26 * d + c.charCodeAt(i) - 64; return d - 1; }
    function encode_col(col) { var s = ""; for (++col; col; col = Math.floor((col - 1) / 26))
    ...
    function format_cell(cell, v, o) {
        ...
        if (cell.t == 'd' && !cell.z && o && o.dateNF)
            cell.z = o.dateNF;
        if (v == undefined)
            return safe_format_cell(cell, cell.v, o);
    ...
    function write_RkNumber(data, o) {
        ...
        if (fInt)
            ...
            throw new Error("unsupported RkNumber " + data); // TODO
    }
    /* [MS-XLSB] 2.5.117 RfX */
    ...
    var CT_LIST = (function () {
        ...
        keys(o).forEach(function (k) { if (!o[k].xlsm)
            o[k].xlsm = o[k].xlsx; });
    ...
    function parse_VtUnalignedString(blob, t) { if (!t)
        throw new Error("dafuq?"); return parse_VtStringBase(blob, t, 0); }
    ...
    function parse_XLUnicodeRichExtendedString(blob) {
        ...
        if (fRichSt)
            blob.l += 4 * cRun; //TODO: parse this
        if (fExtSt)
            blob.l += cbExtRst; //TODO: parse this
        z.t = msg;
        if (!fRichSt) {
            z.raw = "<t>" + z.t + "</t>";
            z.r = z.t;
    ...
    function parse_FtArray(blob, length, ot) {
        ...
        if (blob.l != tgt)
            blob.l = tgt; //throw new Error("bad Object Ft-sequence");
        return fts;
    }
    /* 2.5.129 */
            ...
            while (nrow-- > 0) {
                ...
                for (C = 0; C != fields.length; ++C) {
                    ...
                    switch (fields[C].type) {
                        ...
                        case 'D':
                            if (s.length === 8)
                                out[R][C] = new Date(+s.substr(0, 4), +s.substr(4, 2) - 1, +s.substr(6, 2));
                            else
                                out[R][C] = s;
                            break;
                        case 'F':
    ...
    var PRN = (function () {
        ...
        function dsv_to_sheet_str(str, opts) {
            ...
            else if (str.substr(0, 1024).indexOf("\t") == -1)
                sep = ",";
            else
                sep = "\t";
            var R = 0, C = 0, v = 0;
        ...
        recordhopper(data, function hopper_sty(val, R_n, RT) {
            switch (RT) {
                ...
                case 0x002B:/* 'BrtFont' */ 
                    ...
                    break;
                case 0x0401:/* 'BrtKnownFonts' */  /* 'BrtKnownFonts' */ break;
                case 0x002D:/* 'BrtFill' */  /* 'BrtFill' */ break;
                case 0x002E:/* 'BrtBorder' */  /* 'BrtBorder' */ break;
                case 0x002F:/* 'BrtXF' */ 
        ...
        recordhopper(data, function hopper_cmnt(val, R_n, RT) {
            switch (RT) {
                ...
                case 0x0024:/* 'BrtFRTEnd' */ 
                    pass = false;
                    break;
                case 0x0025:/* 'BrtACBegin' */  /* 'BrtACBegin' */ break;
                case 0x0026:/* 'BrtACEnd' */  /* 'BrtACEnd' */ break;
                default:
                    if ((R_n || "").indexOf("Begin") > 0) { }
                    else if ((R_n || "").indexOf("End") > 0) { }
                    else if (!pass || opts.WTF)
    ...
    (function () { for (var y in PtgDupes)
    ...
    function write_ws_xml_protection(sp) {
        ...
        deffalse.forEach(function (n) { if (sp[n] != null && sp[n])
            o[n] = "1"; });
        deftrue.forEach(function (n) { if (sp[n] != null && !sp[n])
            o[n] = "0"; });
        /* TODO: algorithm */
        if (sp.password)
    ...
    function write_ws_xml(idx, opts, wb, rels) {
        ...
        if (opts.sheetFormat)
            o[o.length] = (writextag('sheetFormatPr', null, { defaultRowHeight: opts.sheetFormat.defaultRowHeight || '16', baseColWidth: opts.sheetFormat.baseColWidth || '10' }));
    ...
    function write_COLINFOS(ba, ws, idx, opts, wb) {
        ...
        ws['!cols'].forEach(function (m, i) { if (m)
        ...
        recordhopper(data, function hopper_wb(val, R_n, RT) {
            switch (RT) {
                ...
                case 0x0027:/* 'BrtName' */ 
                    ...
                    Names.push(val);
                    break;
                case 0x040C:/* 'BrtNameExt' */  /* 'BrtNameExt' */ break;
                ...
                case 0x0025:/* 'BrtACBegin' */  /* 'BrtACBegin' */ break;
                case 0x0026:/* 'BrtACEnd' */  /* 'BrtACEnd' */ break;
                case 0x0010:/* 'BrtFRTArchID$' */  /* 'BrtFRTArchID$' */ break;
                default:
                    if ((R_n || "").indexOf("Begin") > 0) { }
                    else if ((R_n || "").indexOf("End") > 0) { }
                    else if (!pass || opts.WTF)
    ...
    */
            ...
            switch (Rn[3]) {
                case 'Data':
                    if (state[state.length - 1][1])
                        break;
                    if (Rn[1] === '/')
                        parse_xlml_data(str.slice(didx, Rn.index), ss, dtag, state[state.length - 1][0] == "Comment" ? comment : cell, { c: c, r: r }, styles, cstys[c], row, arrayf, opts);
                    else {
                        ss = "";
    ...
    function write_ws_xlml_wsopts(ws, opts, idx, wb) {
        ...
        if (ws['!protect']) {
            ...
            ].forEach(function (x) { if (ws['!protect'][x[0]])
                o.push("<" + x[1] + "/>"); });
        }
        if (o.length == 0)
            return "";
    ...
    function parse_workbook(blob, options) {
            ...
            if (R && R.f) {
                ...
                switch (Rn) {
                    ...
                    case 'Password':
                        ...
                        break;
                    case 'Prot4Rev':
                    case 'Prot4RevPass': break; /*TODO: Revision Control*/
                    case 'BoundSheet8':
                        {
                    ...
                    case 'BOF':
                        {
                            if (opts.biff !== 8) { }
                                ...
                                opts.biff = 3;
                            else if (RecordType === 0x0409)
                                opts.biff = 4;
                            else if (val.BIFFVer === 0x0500)
                                opts.biff = 5;
                            else if (val.BIFFVer === 0x0600)
                                opts.biff = 8;
                    ...
                    default: switch (R.n) {
                        ...
                        case 'InterfaceEnd':
                        case 'DSF':
                        case 'BuiltInFnGroupCount': break;
                        /* View Stuff */
                        case 'Window1':
                        case 'Window2':
                        case 'HideObj':
                        ...
                        default: switch (R.n) {
                            ...
                            case 'ChartFormat':
                            case 'ChartFrtInfo': break;
                            case 'PlotArea':
                            case 'PlotGrowth': break;
                            case 'SeriesList':
                            case 'SerParent':
                            case 'SerAuxTrend': break;
                            case 'DataFormat':
                            case 'SerToCrt':
                            case 'FontX': break;
                            case 'CatSerRange':
                            ...
                            case 'Legend':
                            case 'LegendException': break;
                            case 'Pie':
                            case 'Scatter': break;
                            case 'PieFormat':
                            case 'MarkerFormat': break;
                            case 'StartObject':
                            case 'EndObject': break;
                            case 'AlRuns':
                            case 'ObjectLink': break;
                            case 'SIIndex': break;
                            case 'AttachedLabel':
                            case 'YMult': break;
                            /* Chart Group */
                            ...
                            case 'MsoDrawingSelection': break;
                            /* Pub Stuff */
                            case 'WebPub':
                            case 'AutoWebPub': break;
                            /* Print Stuff */
                            case 'HeaderFooter':
                            case 'HFPicture':
                            case 'PLV':
                            case 'HorizontalPageBreaks':
                            case 'VerticalPageBreaks': break;
                            /* Behavioral */
                            case 'Backup':
                            ...
                            /* Should not Happen */
                            case 'Continue':
                            case 'ContinueFrt12': break;
                            /* Future Records */
                            case 'FrtFontList':
                            case 'FrtWrapper': break;
                            default: switch (R.n) {
                                /* BIFF5 records */
                                ...
                                case 'Intl':
                                case 'CoordList':
                                case 'SerAuxErrBar': break;
                                /* BIFF2-4 records */
                                case 'BIFF2FONTCLR':
                                case 'BIFF2FMTCNT':
                                case 'BIFF2FONTXTRA': break;
                                case 'BIFF2XF':
                                case 'BIFF3XF':
                                case 'BIFF4XF': break;
                                case 'BIFF4FMTCNT':
                                case 'BIFF2ROW':
                                case 'BIFF2WINDOW2': break;
                                /* Miscellaneous */
                                case 'SCENARIO':
                                case 'DConBin':
                                case 'PicF':
        ...
        if (!options.bookSheets)
            wb.Sheets = Sheets;
        if (wb.Sheets)
            FilterDatabases.forEach(function (r, i) { wb.Sheets[wb.SheetNames[i]]['!autofilter'] = r; });
    ...
    function parse_xlscfb(cfb, options) {
        ...
        if (options.bookProps && !options.bookSheets)
            ...
            if (Workbook)
                ...
                WorkbookP = WK_.to_workbook(cfb.find('PerfectOffice_MAIN').content, options);
            else if (cfb.find('NativeContent_MAIN'))
                WorkbookP = WK_.to_workbook(cfb.find('NativeContent_MAIN').content, options);
            else
    ...
    var XLSRecordEnum = {
        ...
        0x001f: { n: "BIFF2FMTCNT", f: parsenoop },
        ...
        0x0056: { n: "BIFF4FMTCNT", f: parsenoop },
        0x007e: { n: "RK", f: parsenoop },
        ...
        return function pcx(d, _opts) {
            ...
            while ((Rn = xlmlregex.exec(str)))
                switch ((Rn[3] = Rn[3].replace(/_.*$/, ""))) {
                    ...
                    case 'body':
                    case '电子表格': break; // 3.3 16.9.6 19.726.3
                    ...
                    default:
                        if (Rn[2] === 'dc:')
                            break; // TODO: properties
                        if (Rn[2] === 'draw:')
                            break; // TODO: drawing
                        if (Rn[2] === 'style:')
                            break; // TODO: styles
                        if (Rn[2] === 'calcext:')
                            break; // ignore undocumented extensions
                        if (Rn[2] === 'loext:')
                            break; // ignore undocumented extensions
                        if (Rn[2] === 'uof:')
                            break; // TODO: uof
    ...
    function safe_parse_wbrels(wbrels, sheets) {
        if (!wbrels)
            return 0;
        try {
            wbrels = sheets.map(function pwbr(w) { if (!w.id)
                w.id = w.strRelID; return [w.name, wbrels['!id'][w.id].Target, get_sheet_type(wbrels['!id'][w.id].Type)]; });
        }
        catch (e) {
            return null;
        }
    ...
    function parse_zip(zip, opts) {
        ...
        if (opts.bookSheets || opts.bookProps) {
            if (wb.Sheets)
                sheets = wb.Sheets.map(function pluck(x) { return x.name; });
    ...
    function write_zip(wb, opts) {
        ...
        if (wb.Props && wb.Props.SheetNames) { }
            ...
            wb.Props.SheetNames = wb.SheetNames.map(function (x, i) { return [(wb.Workbook.Sheets[i] || {}).Hidden != 2, x]; }).filter(function (x) { return x[0]; }).map(function (x) { return x[1]; });
    ...
    function readSync(data, opts) {
        ...
        switch ((n = firstbyte(d, o))[0]) {
            ...
            case 0x03:
            case 0x83:
            case 0x8B: return DBF.to_workbook(d, o);
        }
        if (n[2] <= 12 && n[3] <= 31)
        ...
        switch (opts.type) {
            ...
            case "buffer": {
                if (has_buf)
                    ...
                    return out.split("").map(function (c) { return c.charCodeAt(0); });
            }
        }
        throw new Error("Unrecognized type " + opts.type);
    }
