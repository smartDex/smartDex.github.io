...
AmCharts.addInitHandler(function (chart) {
    ...
    var applyConfig = function (current, override) {
        ...
        for (var property in override) {
            ...
            if (isNullOrUndefined(currentValue)) {
                if (property !== "periodSelector" && property !== "dataSetSelector")
    ...
    var checkRules = function () {
        ...
        for (var i = 0; i < r.rules.length; i++) {
            var rule = r.rules[i];
            var ruleMatches = (isNullOrUndefined(rule.minWidth) || (rule.minWidth <= width)) && (isNullOrUndefined(rule.maxWidth) || (rule.maxWidth >= width)) &&
