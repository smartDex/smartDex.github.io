(function (vendor_qrcode) {
    ...
    var hasCanvas = (function () { var elem = document.createElement('canvas'); return !!(elem.getContext && elem.getContext('2d')); }());
    ...
    function drawModuleDefault(qr, context, settings, left, top, width, row, col) { if (qr.isDark(row, col)) {
    ...
    function createDiv(settings) {
        ...
        var row;
        var col;
        var containerCSS = { position: 'relative', left: 0, top: 0, padding: 0, margin: 0, width: settings_size, height: settings_size };
        var darkCSS = { position: 'absolute', padding: 0, margin: 0, width: moduleSize, height: moduleSize, 'background-color': settings.fill };
        ...
        for (row = 0; row < moduleCount; row += 1) {
            for (col = 0; col < moduleCount; col += 1) {
                if (qr.isDark(row, col)) {
                    jq('<div/>').css(darkCSS).css({ left: offset + col * moduleSize, top: offset + row * moduleSize }).appendTo($div);
                }
            }
        }
        return $div;
    ...
    var defaults = { render: 'canvas', minVersion: 1, maxVersion: 40, ecLevel: 'L', left: 0, top: 0, size: 200, fill: '#000', background: null, text: 'no text', radius: 0, quiet: 0, mode: 0, mSize: 0.1, mPosX: 0.5, mPosY: 0.5, label: 'no label', fontname: 'sans', fontcolor: '#000', image: null };
    jq.fn.qrcode = function (options) { var settings = jq.extend({}, defaults, options); return this.each(function (idx, el) { if (el.nodeName.toLowerCase() === 'canvas') {
    ...
    var qrcode = function () {
        var qrcode = function (typeNumber, errorCorrectLevel) {
            ...
            var setupPositionProbePattern = function (row, col) { for (var r = -1; r <= 7; r += 1) {
        ...
        qrcode.createStringToBytes = function (unicodeData, numChars) {
            var unicodeMap = function () {
                ...
                var read = function () { var b = bin.read(); if (b == -1)
                    throw new Error(); return b; };
                var count = 0;
                var unicodeMap = {};
                while (true) {
                    var b0 = bin.read();
        ...
        var QRUtil = function () {
            ...
            _this.getMaskFunction = function (maskPattern) { switch (maskPattern) {
            ...
            _this.getLengthInBits = function (mode, type) { if (1 <= type && type < 10) {
                switch (mode) {
        ...
        var QRRSBlock = function () {
            ...
            var _this = {};
            var getRsBlockTable = function (typeNumber, errorCorrectLevel) { switch (errorCorrectLevel) {
        ...
        var qrBitBuffer = function () {
            ...
            _this.put = function (num, length) { for (var i = 0; i < length; i += 1) {
        ...
        var qr8BitByte = function (data) { var _mode = QRMode.MODE_8BIT_BYTE; var _data = data; var _bytes = qrcode.stringToBytes(data); var _this = {}; _this.getMode = function () { return _mode; }; _this.getLength = function (buffer) { return _bytes.length; }; _this.write = function (buffer) { for (var i = 0; i < _bytes.length; i += 1) {
        ...
        var byteArrayOutputStream = function () {
            ...
            _this.writeBytes = function (b, off, len) { off = off || 0; len = len || b.length; for (var i = 0; i < len; i += 1) {
                _this.writeByte(b[i + off]);
            } };
            _this.writeString = function (s) { for (var i = 0; i < s.length; i += 1) {
        ...
        var base64EncodeOutputStream = function () {
            ...
            _this.writeByte = function (n) { _buffer = (_buffer << 8) | (n & 0xff); _buflen += 8; _length += 1; while (_buflen >= 6) {
        ...
        var base64DecodeInputStream = function (str) {
            ...
            var decode = function (c) { if (0x41 <= c && c <= 0x5a) {
                return c - 0x41;
            }
        ...
        var gifImage = function (width, height) {
            ...
            var bitOutputStream = function (out) {
                ...
                _this.flush = function () { if (_bitLength > 0) {
